...
description: "Swiss legal document analyzer — reads Swiss legal documents (contracts, court decisions, statutes, submissions, legal opinions) and produces structured analysis: legal issues, key clauses, citation verification, risk flags, and compliance assessment. Includes NDA triage mode (GREEN/YELLOW/RED classification against Swiss law and local playbook) and playbook-aware contract review. Trigger when: a user uploads or pastes a legal document for review; when an agent needs to extract citations or assess a document before drafting; when checking a draft for errors; when the user asks for NDA triage; or when keywords like 'playbook', 'posizioni standard', 'NDA', 'triage' appear. Uses swiss-caselaw and entscheidsuche MCPs for precedent checks; legal-citations MCP for citation extraction and validation; fedlex-sparql for statutory verification. For confidential or privileged documents, activate privacy-routing skill first. If the Anthropic Legal plugin is also installed, this plugin has precedence for documents governed by Swiss law or involving Swiss parties. Do NOT trigger for: creating new documents (use swiss-legal-drafting), citation formatting only (use swiss-citation-formats), or research without a document (use swiss-legal-research)."
---
