...
**Output convention**: Write the drafted document to `bcc-output/YYYY-MM-DD-<slug>/05-draft-<doc>.md` (or `.docx` for redline/tracked changes) and give in chat only a 3–5 line summary with the file path. See `skills/shared/output-conventions.md`. You can also say: "prepara il contratto come file" or "draft it as a document" for file output.
