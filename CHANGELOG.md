...
- **Deliverable-as-file convention** (`bcc-output/`): long outputs (memos, research, strategy, drafts, triage) are now written as files in `bcc-output/YYYY-MM-DD-<slug>/` with numbered phases and `sources.md` for citation trails. Chat shows only a 3–5 line summary. Output folder configurable in playbook. Defined once in `skills/shared/output-conventions.md` and referenced by all commands.
